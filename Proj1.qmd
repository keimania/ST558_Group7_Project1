...
summary.census <- function(cen_tbl,num_vars=numeric_vars, cat_vars=categorical_vars ){
  ...
  for (num_var in num_vars){
    ...
    num_var_col_mean <- sum(num_var_col * census$PWGTP, na.rm = TRUE) / 
                    sum(census$PWGTP[!is.na(num_var_col)])
    
    ## 
    ##num_var_col_sd<-(sd(as.numeric(num_var_col, na.rm = TRUE)))
    
    num_var_col_sd <- sqrt(sum((num_var_col^2) * census$PWGTP, na.rm = TRUE) / 
    sum(census$PWGTP[!is.na(num_var_col)]) -num_var_col_mean^2)
