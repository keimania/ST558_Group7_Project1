...
# Querying the PUMS Census API

## Introduction

The goal of this project is to create some functions to query the PUMS census API, and then to summarize and visualize queries.

Key functionalities developed include:

-   A `helper` function to query the API for a single year, with parameters for specific numeric and categorical variables, as well as geographical levels (state, region, etc.).

-   A `multiple_years_helper` function that extends this capability to retrieve and combine data across several years.

-   Custom S3 methods, `summary.census` and `plot.census`, designed to provide tailored summaries and visualizations for the data retrieved from the API.

After building these functions, we will then use them to investigate the relationship between location, gross rent as a percentage of income, and sex.
...
We would also like to be able to specify the geography level. The possible values for this are All, Region, Division, or State. The user can then specify which region, division or state they would like to return information for using the geo_level parameter.

Furthermore, we will also allow the user to specify an optional argument to subset the data. Parameter 'census_lookups' will be used to make the mapping table for 'JWAP' and 'JWDP' columns.
...
Second, we build functions to handle time-based variables like JWAP (Time of arrival at work) and JWDP (Time of departure from work). The API returns these as coded values representing time ranges (e.g., "9:40 p.m. to 9:44 p.m."). The time_range_to_midpoint function parses these strings and calculates the midpoint, returning it as a standardized hms object for easier computation.
...
Third, we will build a helper function to take what is returned by GET() and turn it into a tibble. We are also building in all the requirements specified at the beginning of the data processing section.
...
Here, we test the helper function with two different single-year queries to ensure it behaves as expected. The first query pulls data for a specific region and educational level in 2021 , while the second retrieves data for a state based on transportation mode in 2022.
...
Now we define summary.census, our custom summary method. This function is designed to work specifically with objects of the "census" class.
...
-   A count table is created for categorical variables. For each numeric variable, it calculates the weighted mean and standard deviation, using the person weight variable PWGTP to ensure the statistics are representative of the population.
...
summary.census <- function(cen_tbl,num_vars=numeric_vars, cat_vars=categorical_vars ){
  ...
  for (num_var in num_vars){
    ...
    # get weighted mean and SD of col, ignore NA values
    #denominator of this checks to look for na values.
    
    num_var_col <- as.numeric(num_var_col)
    
    num_var_col_mean <- sum(num_var_col * cen_tbl$PWGTP, na.rm = TRUE) / 
                    sum(cen_tbl$PWGTP[!is.na(num_var_col)])
    
    ##num_var_col_sd<-(sd(as.numeric(num_var_col, na.rm = TRUE)))
    
    num_var_col_sd <- sqrt(sum((num_var_col^2) * cen_tbl$PWGTP, na.rm = TRUE) / 
    sum(cen_tbl$PWGTP[!is.na(num_var_col)]) -num_var_col_mean^2)
    
    ## Convert back to time type if necessary
if (num_var == "JWAP" || num_var == "JWDP") {
  num_var_col_mean <- as.hms(num_var_col_mean)
}
...
Next, let's test it out, I'll specifying some variables.
...
We now create a custom plot.census method to generate box plots that visualize the relationship between a specified categorical and numeric variable.
...
## Data Analysis: Housing Affordability in NY vs. NC
...
In New York, the average gross rent as a percentage of household income (GRPIP) is marginally in 2022 than 2010. This is surprising--there is a lot of discourse about rising rents in New York and in the United States in general. However, New Yorkers are still spending more on their rents on average than North Carolinians.
